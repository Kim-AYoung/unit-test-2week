...
@RunWith(MockitoJUnitRunner.class) // test 환경의 기능 확장이나 초기화
public class MockServiceTest {

    @Mock // mock(MockRepository.class)로도 가능
    ...
    public void 챔피언이름을가져오면_무조건_카이사를_리턴한다() {
        Champion champion = mock(Champion.class); // 가짜 champion 객체

        // mock obj의 setName()을 호출해도 setting 안됨
    ...
    // 1-1. 어떠한 포지션을 set 해도 미드를 리턴하도록 테스트하세요
    @Test
    ...
    public void shouldOneTimeRunForChampionArraySize() {
        ...
        System.out.println("size :: " + mockChampions.size()); // 가짜 객체이기때문에
    ...
    public void shouldPassWhenNeverRunChampionSetName() {
        ...
        verify(champion, never()).setName(anyString()); // any(String.class)
    }
    ...
    public void 챔피언정보들을Mocking하고Service메소드호출테스트() {
        ...
        System.out.println("챔피언 이름 : " + championName);
    ...
    public void List객체사이즈검증테스트() {
        ...
        //verify(mockService, times(1)).findAllChampions(); // 오류
    }
    ...
    public void 가짜챔피언객체리턴테스트() {
        when(mockService.findByName(anyString())).thenReturn(new Champion("리산드라", "미드", 1));
    ...
    public void 여러개이상호출회수검증() {
        ...
        assertThat(mockService.findAllChampions().get(1).getHasSkinCount(), is(1));
